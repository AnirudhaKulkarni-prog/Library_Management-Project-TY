...
public class Forgot_Librarian extends JFrame implements ActionListener{
	...
	new Forgot_Librarian().setVisible(true);
    }

    public Forgot_Librarian() {
    ...
    public void actionPerformed(ActionEvent ae){
        try{
            conn con = new conn();
            if(ae.getSource() == b1){
                String sql = "select * from librarian_account where username=?";
		...
		while (rs.next()) {
            ...
            if(ae.getSource() == b2){
                String sql = "select * from librarian_account where sec_ans=?";
		PreparedStatement st = con.c.prepareStatement(sql);
