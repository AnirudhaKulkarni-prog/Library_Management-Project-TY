...
public class Librarian_Login extends JFrame implements ActionListener{
	...
	l7.setFont(new Font("Trebuchet MS", Font.BOLD, 18));
	ImageIcon i7 = new ImageIcon(ClassLoader.getSystemResource("library/management/system/icons/tenth.png"));
        Image i8 = i7.getImage().getScaledInstance(20, 20, Image.SCALE_DEFAULT);
        ImageIcon i9 = new ImageIcon(i8);
        l7.setIcon(i9);
	l7.setBounds(40, 30, 72, 33);
	panel.add(l7);
	
	 JLabel l6 = new JLabel("LIBRARIAN LOGIN");
     l6.setForeground(new Color(204, 51, 102));
     l6.setFont(new Font("Segoe UI Semilight", Font.BOLD, 15));
     l6.setBounds(230, 50, 150, 30);
     panel.add(l6);
	...
	panel.add(l5);
		...
		try {
                    conn con = new conn();
                    String sql = "select * from librarian_account where username=? and password=?";
            ...
            if(ae.getSource() == b2){
                setVisible(false);
                Signup_Librarian su = new Signup_Librarian();
		su.setVisible(true);
            }   
            if(ae.getSource() == b3){
                setVisible(false);
